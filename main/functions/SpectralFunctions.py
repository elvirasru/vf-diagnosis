...
def bwt(sub_signal, frequency):
    ...
    x_d = np.ones(1, window_duration * frequency) * sub_signal[:], np.transpose(
        np.ones(1, window_duration * frequency) * sub_signal[0:length -1])
    b, a = scipy.signal.butter(5, 2 * [6.5, 30] / frequency, btype="bandpass")
    x_d = scipy.signal.filtfilt(b, a, x_d)
    x_d(window_duration * frequency + x_d[0:] - window_duration * frequency)
